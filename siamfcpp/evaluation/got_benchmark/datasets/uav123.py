...
class UAV123(object):
    ...
    def __init__(self, root_dir, version='UAV123'):
        ...
        assert version.upper() in ['UAV20L', 'UAV123', 'UAV123@10FPS']
        ...
        #glob.glob('/home/lsw/PycharmProjects/siamfc_Fisher/siamfc_6/datasets/UAV123@10FPS/anno/UAV123_10fps/*.txt')
        self.anno_files = sorted(glob.glob(
            os.path.join(root_dir, 'anno/%s/*.txt' % version)))

        if self.version == 'UAV123@10FPS':
            self.seq_names = [
                os.path.basename(f)[:-4] for f in self.anno_files]
            self.seq_dirs = [os.path.join(
                root_dir, self.seq_metas[version][n]['folder_name'])
            ...
            if self.version == 'UAV123@10FPS':
                for seq_name in seq_names:
                    seq_dir = os.path.join(
                        root_dir, seq_metas[version][seq_name]['folder_name'])
